...
class HPacker(FullStructureReconstructor):
    ...
    def __init__(self,
                 model_dirs: List[str],
                 *args, 
                 charges_filepath: Optional[str] = None,
        ...
        #     self.standardize_original_structure()
    ...
    def initial_guess(self, batch_size=1, use_downsampled_model=False, res_ids=None, **kwargs):
        if use_downsampled_model:
            assert res_ids is not None
            rng = np.random.default_rng(123456890)
            rng.shuffle(res_ids)

            res_id_to_chi_angles = {}
            for res_id in tqdm(res_ids):
                predicted_chi_angles = self.add_sidechain(res_id)
                if predicted_chi_angles is not None:
                    res_id_to_chi_angles[res_id] = predicted_chi_angles
    ...
    def compute_error_per_chi_angle(self,
        ...
        return mae_per_angle_4, accuracy_per_angle_4, real, predicted, aas
    ...
    def _get_chi_angles_our_way(self, residue: Residue):
        ...
        '''
        from protein_holography_pytorch.preprocessing_faster.utils.structural_info import VEC_AA_ATOM_DICT
    ...
    def populate_sidechains(self,
                            ...
                            batch_size: int = 128,
                            use_downsampled_model: bool = False,
                            standardize_original_structure: bool = False):
        '''
        ...
        seed is used to generate the ordering of res_ids for reconstruction, as random reconstruction is better than sequential reconstruction (as per DLPacker)
        '''
        ...
        start = time.time()
        initial_guess_res_id_to_angles = self.initial_guess(batch_size=batch_size, use_downsampled_model=use_downsampled_model, res_ids=list(real_res_id_to_angles.keys()))
        print('Initial guess took %.2f seconds' % (time.time() - start))

        initial_guess_mae_per_angle_4, initial_guess_accuracy_per_angle_4, initial_guess_real, initial_guess_predicted, initial_guess_aas = self.compute_error_per_chi_angle(real_res_id_to_angles, initial_guess_res_id_to_angles, res_id_to_centrality)
        ...
        for i in range(num_refinement_iterations):
            print(f'Iteration {i+1}/{num_refinement_iterations}')

            start = time.time()
            refined_res_id_to_angles = self.refinement(batch_size=batch_size)
            elapsed = time.time() - start
            time_refinement += elapsed
            print('Refinement for %d times took %.2f seconds' % (i+1, time_refinement))

            mae_per_angle_4, accuracy_per_angle_4, real, predicted, aas = self.compute_error_per_chi_angle(real_res_id_to_angles, refined_res_id_to_angles, res_id_to_centrality)
            self.pretty_print_error_per_chi_angle(mae_per_angle_4, accuracy_per_angle_4)

            if i == 1:
                intermediate_mae_per_angle_4, intermediate_accuracy_per_angle_4, intermediate_real, intermediate_predicted, intermediate_aas = deepcopy(mae_per_angle_4), deepcopy(accuracy_per_angle_4), deepcopy(real), deepcopy(predicted), deepcopy(aas)
                intermediate_rmsds = self.get_rmsds(res_id_to_centrality, standardize_original_structure=standardize_original_structure)

        refined_rmsds = self.get_rmsds(res_id_to_centrality, standardize_original_structure=standardize_original_structure)


        return (mae_per_angle_4, accuracy_per_angle_4, real, predicted, aas, refined_rmsds), (initial_guess_mae_per_angle_4, initial_guess_accuracy_per_angle_4, initial_guess_real, initial_guess_predicted, initial_guess_aas, initial_guess_rmsds), (intermediate_mae_per_angle_4, intermediate_accuracy_per_angle_4, intermediate_real, intermediate_predicted, intermediate_aas, intermediate_rmsds)
...
if __name__ == '__main__':
    ...
    if native:
        ...
        pdb_lists = {
            'CASP13': '/gscratch/scrubbed/gvisan01/dlpacker/pdb_lists/casp13_FM_targets_testing.txt',
            'CASP14': '/gscratch/scrubbed/gvisan01/dlpacker/pdb_lists/casp14_FM_targets_testing.txt',
        }
        pdbdir = '/gscratch/scrubbed/gvisan01/dlpacker/non_native/'

    batch_size = 512

    charges_filepath = '/gscratch/spe/gvisan01/hpacker/src/preprocessing/utils/charges.rtp'
    ...
    for dataset in ['CASP13', 'CASP14']:
        ...
        mae_per_angle_trace = []
        accuracy_per_angle_trace = []
        real_trace = []
        predicted_trace = []
        aas_trace = []
        refined_rmsds_trace = []

        initial_guess_mae_per_angle_trace = []
        initial_guess_accuracy_per_angle_trace = []
        initial_guess_real_trace = []
        initial_guess_predicted_trace = []
        initial_guess_aas_trace = []
        initial_guess_rmsds_trace = []

        intermediate_mae_per_angle_trace = []
        intermediate_accuracy_per_angle_trace = []
        intermediate_real_trace = []
        intermediate_predicted_trace = []
        intermediate_aas_trace = []
        intermediate_refined_rmsds_trace = []

        for i, pdb in enumerate(pdbs):
            print(f'{i+1}/{len(pdbs)} - {pdb}')
            pdbpath = os.path.join(pdbdir, pdb + '.pdb')

            fsr = HPacker(model_dirs, pdbpath, charges_filepath=charges_filepath, remove_sidechains=True, virtual_CB=virtual_CB)
            ...
            try:
                ...
                (intermediate_mae_per_angle_4, intermediate_accuracy_per_angle_4, intermediate_real, intermediate_predicted, intermediate_aas, intermediate_rmsds) = fsr.populate_sidechains(seed=42, batch_size=batch_size, use_downsampled_model=use_downsampled_model, standardize_original_structure=standardize_original_structure)
            except AssertionError:
                print('Skipping', pdb)
                continue

            mae_per_angle_trace.append(mae_per_angle_4.numpy())
            accuracy_per_angle_trace.append(accuracy_per_angle_4.numpy())
            real_trace.append(real)
            predicted_trace.append(predicted)
            aas_trace.append(aas)
            refined_rmsds_trace.append(refined_rmsds)

            initial_guess_mae_per_angle_trace.append(initial_guess_mae_per_angle_4.numpy())
            initial_guess_accuracy_per_angle_trace.append(initial_guess_accuracy_per_angle_4.numpy())
            initial_guess_real_trace.append(initial_guess_real)
            initial_guess_predicted_trace.append(initial_guess_predicted)
            initial_guess_aas_trace.append(initial_guess_aas)
            initial_guess_rmsds_trace.append(initial_guess_rmsds)

            intermediate_mae_per_angle_trace.append(intermediate_mae_per_angle_4.numpy())
            intermediate_accuracy_per_angle_trace.append(intermediate_accuracy_per_angle_4.numpy())
            intermediate_real_trace.append(intermediate_real)
            intermediate_predicted_trace.append(intermediate_predicted)
            intermediate_aas_trace.append(intermediate_aas)
            intermediate_refined_rmsds_trace.append(intermediate_rmsds)

        print('----- INITIAL GUESS SCORES -----')
        initial_guess_mae_per_angle_all, initial_guess_accuracy_per_angle_all, initial_guess_global_accuracy_all, initial_guess_mae_per_aa_all, initial_guess_error_per_aa_all = loss_per_chi_angle(torch.tensor(np.vstack(stack_dicts(initial_guess_predicted_trace)['all'])), torch.tensor(np.vstack(stack_dicts(initial_guess_real_trace)['all'])), torch.tensor(np.hstack(stack_dicts(initial_guess_aas_trace)['all'])), return_global_accuracy=True, return_per_aa=True)
        initial_guess_mae_per_angle_core, initial_guess_accuracy_per_angle_core, initial_guess_global_accuracy_core = loss_per_chi_angle(torch.tensor(np.vstack(stack_dicts(initial_guess_predicted_trace)['core'])), torch.tensor(np.vstack(stack_dicts(initial_guess_real_trace)['core'])), torch.tensor(np.hstack(stack_dicts(initial_guess_aas_trace)['core'])), return_global_accuracy=True)
        initial_guess_mae_per_angle_surface, initial_guess_accuracy_per_angle_surface, initial_guess_global_accuracy_surface = loss_per_chi_angle(torch.tensor(np.vstack(stack_dicts(initial_guess_predicted_trace)['surface'])), torch.tensor(np.vstack(stack_dicts(initial_guess_real_trace)['surface'])), torch.tensor(np.hstack(stack_dicts(initial_guess_aas_trace)['surface'])), return_global_accuracy=True)
        pretty_print_scores(dataset, initial_guess_accuracy_per_angle_all, initial_guess_mae_per_angle_all, initial_guess_global_accuracy_all, initial_guess_global_accuracy_core, initial_guess_global_accuracy_surface, initial_guess_aas_trace, initial_guess_accuracy_per_angle_trace, initial_guess_mae_per_angle_trace, stack_dicts(initial_guess_rmsds_trace))
        print()
        print('----- INTERMEDIATE SCORES -----')
        intermediate_mae_per_angle_all, intermediate_accuracy_per_angle_all, intermediate_global_accuracy_all, intermediate_mae_per_aa_all, intermediate_error_per_aa_all = loss_per_chi_angle(torch.tensor(np.vstack(stack_dicts(intermediate_predicted_trace)['all'])), torch.tensor(np.vstack(stack_dicts(intermediate_real_trace)['all'])), torch.tensor(np.hstack(stack_dicts(intermediate_aas_trace)['all'])), return_global_accuracy=True, return_per_aa=True)
        intermediate_mae_per_angle_core, intermediate_accuracy_per_angle_core, intermediate_global_accuracy_core = loss_per_chi_angle(torch.tensor(np.vstack(stack_dicts(intermediate_predicted_trace)['core'])), torch.tensor(np.vstack(stack_dicts(intermediate_real_trace)['core'])), torch.tensor(np.hstack(stack_dicts(intermediate_aas_trace)['core'])), return_global_accuracy=True)
        intermediate_mae_per_angle_surface, intermediate_accuracy_per_angle_surface, intermediate_global_accuracy_surface = loss_per_chi_angle(torch.tensor(np.vstack(stack_dicts(intermediate_predicted_trace)['surface'])), torch.tensor(np.vstack(stack_dicts(intermediate_real_trace)['surface'])), torch.tensor(np.hstack(stack_dicts(intermediate_aas_trace)['surface'])), return_global_accuracy=True)
        pretty_print_scores(dataset, intermediate_accuracy_per_angle_all, intermediate_mae_per_angle_all, intermediate_global_accuracy_all, intermediate_global_accuracy_core, intermediate_global_accuracy_surface, intermediate_aas_trace, intermediate_accuracy_per_angle_trace, intermediate_mae_per_angle_trace, stack_dicts(intermediate_refined_rmsds_trace))
        print()
        print('----- FINAL SCORES -----')
        mae_per_angle_all, accuracy_per_angle_all, global_accuracy_all, mae_per_aa_all, error_per_aa_all = loss_per_chi_angle(torch.tensor(np.vstack(stack_dicts(predicted_trace)['all'])), torch.tensor(np.vstack(stack_dicts(real_trace)['all'])), torch.tensor(np.hstack(stack_dicts(aas_trace)['all'])), return_global_accuracy=True, return_per_aa=True)
        mae_per_angle_core, accuracy_per_angle_core, global_accuracy_core = loss_per_chi_angle(torch.tensor(np.vstack(stack_dicts(predicted_trace)['core'])), torch.tensor(np.vstack(stack_dicts(real_trace)['core'])), torch.tensor(np.hstack(stack_dicts(aas_trace)['core'])), return_global_accuracy=True)
        mae_per_angle_surface, accuracy_per_angle_surface, global_accuracy_surface = loss_per_chi_angle(torch.tensor(np.vstack(stack_dicts(predicted_trace)['surface'])), torch.tensor(np.vstack(stack_dicts(real_trace)['surface'])), torch.tensor(np.hstack(stack_dicts(aas_trace)['surface'])), return_global_accuracy=True)
        pretty_print_scores(dataset, accuracy_per_angle_all, mae_per_angle_all, global_accuracy_all, global_accuracy_core, global_accuracy_surface, aas_trace, accuracy_per_angle_trace, mae_per_angle_trace, stack_dicts(refined_rmsds_trace))


        for aa_ind in mae_per_aa_all:
            aa = one_letter_to_aa[ind_to_ol_size[aa_ind]]
            print('%s:\t%.2f\t%.2f\t%.2f\t%.2f' % (aa, mae_per_aa_all[aa_ind][0].item(), mae_per_aa_all[aa_ind][1].item(), mae_per_aa_all[aa_ind][2].item(), mae_per_aa_all[aa_ind][3].item()))
        print()
        print()

        import gzip, pickle

        with gzip.open(f'{dataset}__initial_guess_error_per_aa_all.pkl.gz', 'wb') as f:
            pickle.dump(initial_guess_error_per_aa_all, f)

        with gzip.open(f'{dataset}__intermediate_error_per_aa_all.pkl.gz', 'wb') as f:
            pickle.dump(intermediate_error_per_aa_all, f)
        
        with gzip.open(f'{dataset}__error_per_aa_all.pkl.gz', 'wb') as f:
            pickle.dump(error_per_aa_all, f)
